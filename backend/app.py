...
def request_reset():
    try:
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT userID FROM usercredentials WHERE email = %s", (email,))
        user = cursor.fetchone()
        ...
        user_id = user[0]
        reset_token = jwt.encode({"userID": user_id, "exp": datetime.utcnow() + timedelta(minutes=15)},
